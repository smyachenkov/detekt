 ...
 * </compliant>
 */
class ExplicitMapElementAccessMethod(config: Config = Config.empty) : Rule(config) {
    ...
    private fun isMapMethod(expression: KtCallExpression): Boolean {
        ...
        standardTypeName?.let {
            return it.toString() == "Map"
        }
        val returnTypes = callerReturnType?.collectTypes()
        return returnTypes?.any { it.constructor.toString() in setOf("Map", "AbstractMap") } ?: false
    }
